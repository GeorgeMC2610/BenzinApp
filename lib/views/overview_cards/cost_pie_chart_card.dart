...
class CostPieChartCard extends StatefulWidget {
  ...
  @override
  createState() => _CostPieChartCardState();
}
...
class _CostPieChartCardState extends State<CostPieChartCard> {
        ...
        child: Card(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Center(
                    child: AutoSizeText(translate('combinedCosts'),
                        maxLines: 1,
                        ...
                        Row(
                          ...
                          children: [
                            Text(
                              "◉ ",
                              style: TextStyle(
                                  color: CostsPieChart.pieChartColors[
                                      value.themeMode.name]!['fuel'],
                                  fontSize: 25),
                            ),
                            Text(
                                '${translate('fuelFills')} - ${CarManager().watchingCar!.toCurrency(LocaleStringConverter.formattedDouble(context, totalFuelFillCosts!))}')
                          ],
                        ),
                        Row(
                          crossAxisAlignment: CrossAxisAlignment.center,
                          children: [
                            Text(
                              ...
                              style: TextStyle(
                                  color: CostsPieChart.pieChartColors[
                                      value.themeMode.name]!['malfunction'],
                                  fontSize: 25),
                            ),
                            Text(
                                '${translate('malfunctions')} - ${CarManager().watchingCar!.toCurrency(LocaleStringConverter.formattedDouble(context, totalMalfunctionCosts!))}')
                          ],
                        ),
                        Row(
                          crossAxisAlignment: CrossAxisAlignment.center,
                          children: [
                            Text(
                              ...
                              style: TextStyle(
                                  color: CostsPieChart.pieChartColors[
                                      value.themeMode.name]!['service'],
                                  fontSize: 25),
                            ),
                            Text(
                                '${translate('services')} - ${CarManager().watchingCar!.toCurrency(LocaleStringConverter.formattedDouble(context, totalServiceCosts!))}')
                          ],
                        ),
                      ],
                    );
  ...
  Widget build(BuildContext context) =>
      Consumer3<FuelFillRecordManager, MalfunctionManager, ServiceManager>(
        builder: (context, fuelManager, malfunctionManager, serviceManager, _) {
          if (fuelManager.local == null ||
              malfunctionManager.local == null ||
              serviceManager.local == null) {
